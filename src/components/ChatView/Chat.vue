...
const { SendMessageToConversation, UpdateMessageInConversation, DeleteMessageFromConversation, MarkUserAsTyping } = ChatComposable();
...
const props = defineProps({
  ...
  whoIsTyping: { type: Array, required: false }
});
...
let typingTimeout = null;  // Timeout to stop typing notification after a delay
...
function handleTyping() {
  // Emit the typing event when user types
  MarkUserAsTyping(chatID.value, userID, true);

  // Reset typing timeout to stop typing after a delay
  if (typingTimeout) {
    clearTimeout(typingTimeout);
  }
  typingTimeout = setTimeout(() => {
    MarkUserAsTyping(chatID.value, userID, false);
...
<template>
  <div class="chat-component">
      ...
      <div class="messages" v-if="conversationMembers && conversationMessages">
        <template v-for="(message, index) in conversationMessages" :key="index">
          <div class="message-ctn" :class="index + 1 === conversationMessages.length ? 'latest' : null">
            <div class="pfp-container">
              <div class="pfp">
                <img v-if="conversationMembers[message.sender] && conversationMembers[message.sender].photoURL"
                     :src="conversationMembers[message.sender].photoURL" alt="">
    ...
    <!-- Typing Indicator -->
    <div v-if="props.whoIsTyping && props.whoIsTyping.length > 0" class="typing-indicator">
      {{ props.whoIsTyping.join(', ') }} is typing...
    </div>
    ...
    <div class="chat-footer">
      ...
      <div class="chat-input-ctn">
        ...
        <div class="text-area">
          <input class="chat-area" v-model="newMessage" @input="handleTyping" @keyup.enter="sendMessage" placeholder="Tapez votre message..." />
        </div>
